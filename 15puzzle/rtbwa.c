...
long int rtbwa_updates;
long int rtbwa_searches;
long int rtbwa_steps;
long int rtbwa_back_steps;
...
void set_root(Node *node) {
  ...
  node->key[0]=node->h;  
  node->key[1]=0;
  node->search=rtbwa_searches;
  node->back=NULL;
}

long int rtbwa(int start_state[SIZE], int blank)
{
   ...
   initialize_state(node,rtbwa_searches);
   set_root(node);
   empty_heap(open);
   ...
   while (node->h0!=0) {
     ...
     else {   // the node is not in the current path

       ++not_in_path;
       //       threshold=100000;

       if (not_in_path > threshold) {
	
	 fstar[rtbwa_searches]=best->key[0];    // remember best's f value
	 ++rtbwa_searches;                     // increase the search counter
	 assert(rtbwa_searches<MAX_SEARCHES); 
	 ...
	 //	 if (! (best->h < node->h) )  // increment threshold if best is not better than node
   ...
   return 0;//solution_size_no_cycles(start_node);
}
...
int main(int argc, char **argv)
{
  ...
  for (problem = 1; problem <= NUMBER; problem++){ /* for each initial state */
    ...
    add_record_tb(size,total_expansions,rtbwa_steps,rtbwa_back_steps,rtbwa_updates,hashcount,hashmaxlength,thistime,rtbwa_searches);

#ifndef SUCCINCT
    printf ("%3d %7ld %7ld %7ld %7ld %8ld %6ld %6lld %6lld %3.2f\n",problem,size,rtbwa_steps,rtbwa_back_steps,rtbwa_searches,total_expansions,rtbwa_updates,hashcount,hashmaxlength,thistime);
#endif
  }
  print_stats_tb();
